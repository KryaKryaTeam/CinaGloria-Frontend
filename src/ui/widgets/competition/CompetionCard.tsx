...
> = {
  ...
  [CompetitionStatus.STARTED]: {
    label: "Live",
    className: "bg-violet-100 text-violet-700 hover:bg-violet-100 dark:bg-violet-900/30 dark:text-violet-400",
  },
...
const fmt = (d: Date): string =>
  format(d, "d MMM yyyy", { locale: enGB });
...
export function CompetitionCard({ competition }: CompetitionCardProps) {
  ...
  const startDate = dateOfStart instanceof Date ? dateOfStart : new Date(dateOfStart);
  const endDate = dateOfEnd instanceof Date ? dateOfEnd : new Date(dateOfEnd);
  const regStartDate = dateOfStartRegistration instanceof Date
    ? dateOfStartRegistration
    : new Date(dateOfStartRegistration);
  const regEndDate = dateOfEndRegistration instanceof Date
    ? dateOfEndRegistration
