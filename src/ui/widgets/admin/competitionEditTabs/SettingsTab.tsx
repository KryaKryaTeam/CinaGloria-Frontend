...
export const CompetitionEditSettingsTab = observer(() => {
  const { settings } = useChangeCompetitionForm();
  return (
    <TabsContent value="settings" className="space-y-6 outline-none">
      <form onSubmit={settings.submit} className="space-y-6">
            ...
            <Field className="flex flex-row items-center justify-between rounded-lg border border-slate-100 p-4 bg-slate-50/30">
              <div className="space-y-0.5">
                ...
                <FieldDescription className="text-slate-500 max-w-100">
                  If enabled, participants will only see the current active
                  round. Next rounds will remain hidden until the previous one
                  ends.
                </FieldDescription>
              </div>
              <Controller
                name="showRoundsOneByOne"
                control={settings.control}
                render={({ field }) => (
                  <Switch
                    defaultChecked={false}
                    checked={field.value}
                    onCheckedChange={(chk) => field.onChange(chk)}
                  />
                )}
              ></Controller>
              <ErrorBlank error={settings.errors.showRoundsOneByOne} />
            </Field>
          </div>
        </GridCard>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-4">
              <Field>
                ...
                <ErrorBlank error={settings.errors.maxTeams} />
              </Field>
              ...
              <Field>
                ...
                />
                <FieldDescription>
                  How many teams will be displayed as winners on the
                  leaderboard.
                </FieldDescription>
                <ErrorBlank error={settings.errors.showRoundsOneByOne} />
              </Field>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field>
                    ...
                    <Input
                      ...
                      defaultValue={1}
                      min={1}
                      {...settings.register("minTeamMembers", {
                        valueAsNumber: true,
                      })}
                      className="bg-white border-slate-200"
                    />
                    <ErrorBlank error={settings.errors.minTeamMembers} />
                  </Field>
                  ...
                  <Field>
                    ...
                    <Input
                      ...
                      defaultValue={10}
                      min={1}
                      {...settings.register("maxTeamMembers", {
                        valueAsNumber: true,
                      })}
                      className="bg-white border-slate-200"
                    />
                    <ErrorBlank error={settings.errors.maxTeamMembers} />
                  </Field>
                </div>
                <p className="text-[10px] text-slate-400 italic">
                  * These limits will be enforced during the registration
                  process.
                </p>
              </div>
            </div>
