...
export default class CompetitionRule extends ValueObject<ICompetitionRule> { 
    ...
    throw new Error("Description is required and must be less than 1000 characters");
    if (!icon) 
    ...
    equals(other: ValueObject<ICompetitionRule>): boolean { 
        return other instanceof CompetitionRule && this.name === other.name && this.description === other.description && this.icon === other.icon;
    }
